    ...
    <div class="w-full h-full show-chapter-verses">
        ...
        <div id="view-verses-container"
            ...
            " class="sticky top-0 flex w-full mx-auto gap-20 dark:bg-dark-400 bg-white z-9 py-2">
                <div v-for="version in bibleStore.renderVerses[0].version" :key="version.key"
            ...
            <div v-for="verse in bibleStore.renderVerses" :key="verse.verse"
                ...
                <div class="mx-10px">
                    <div :id="verse.verse == bibleStore.selectedVerse ? 'the-selected-verse' : ''" :class="{
                    ...
                    }" :data-book="verse.book_number" :data-chapter="verse.chapter" :data-verse="verse.verse" :style="`border: 1px solid ${clipNoteRender(
                        `key_${verse.book_number}_${verse.chapter}_${verse.verse}`
                    ).color
                        }`"
                        ...
                        <div class="flex flex-row gap-3 w-full"
                            ...
                            <div v-for="version in verse.version" :key="version.key"
                                :style="`width: calc(100%/${bibleStore.selectedBibleVersions.length > 3 ? 1 : bibleStore.selectedBibleVersions.length});`"
                                @contextmenu="clickContextMenu({ ...verse, key: version.key })">
                                <div v-if="bibleStore.selectedBibleVersions.length > 3"
                                ...
                                <div :style="`font-size:${fontSize}px`">
                                    ...
                                    <span :data-bible-version="version.version" :data-book="verse.book_number"
                                        :data-chapter="verse.chapter" :data-key="version.key" :data-verse="verse.verse"
                                        :onfocus="checkHere" class="verse-select-text input-text-search"
                                        contenteditable="true" spellcheck="false" v-html="version.text"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    " :style="`background: ${clipNoteRender(
                        `key_${verse.book_number}_${verse.chapter}_${verse.verse}`
                    ).color
                        ...
                        <div class="absolute flex gap-2 -top-5 right-3">
                            <NButton class="shadow-md" :style="`background: ${clipNoteRender(
                                `key_${verse.book_number}_${verse.chapter}_${verse.verse}`
                            ).color
                                }`" size="small" @click="
                                    createClipNoteRef &&
                        ...
                        <div :style="`font-size:${fontSize - 1}px`" class="px-10px pb-1 view-verse-rendered-clip-note"
                            v-html="clipNoteRender(
                                `key_${verse.book_number}_${verse.chapter}_${verse.verse}`
                            ).content
                                "></div>
                    </div>
        ...
        <ContextMenu :data="contextMenuData" :show-context-menu="showContextMenu" :x="contextMenuPositionX"
            :y="contextMenuPositionY" @close="showContextMenu = false" @create-clip-note="
