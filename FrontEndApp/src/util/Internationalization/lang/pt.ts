import { InternationalMessageType } from './InternationalMessage';

const message: InternationalMessageType = {
    title: `Belivers Sword`,
    'read-bible': 'Ler a Bíblia',
    Sermons: 'Reflexões',
    'Prayer List': 'Lista de orações',
    About: 'Sobre',
    Profile: 'Perfil',
    Settings: 'Configurações',
    'Search Bible': 'Pesquisar na Bíblia',
    'Bible List': 'Lista de Bíblias',
    Bookmarks: 'Marcadores',
    Search: 'Pesquisar',
    'Are You sure?': 'Você tem certeza?',
    Before: 'Anterior',
    Next: 'Próximo',
    'Bible Versions': 'Versões Bíblicas',
    Highlights: 'Destaques',
    Nightly: 'Noturno',
    copy: 'Copiar',
    remove: 'Remover',
    'Clip Notes': 'Notas adesivas',
    note: 'Nota',
    'create new': 'Criar novo',
    close: 'Fechar',
    'prayer list': 'Orações',
    new: 'Nova',
    done: 'Concluído',
    edit: 'Editar',
    create: 'Criar',
    'save changes': 'Salvar mudanças',
    cancel: 'Cancelar',
    'are you sure to remove this item': 'Você tem certeza que deseja remover este item?',
    about: 'sobre',
    donate: 'doar',
    dark: 'escuro',
    light: 'claro',

    /**
     * Bible Versions
     */
    Genesis: 'Gênesis',
    Exodus: 'Êxodo',
    Leviticus: 'Levítico',
    Numbers: 'Números',
    Deuteronomy: 'Deuteronômio',
    Joshua: 'Josué',
    Judges: 'Juízes',
    Ruth: 'Rute',
    '1 Samuel': '1 Samuel',
    '2 Samuel': '2 Samuel',
    '1 Kings': '1 Reis',
    '2 Kings': '2 Reis',
    '1 Chronicles': '1 Crônicas',
    '2 Chronicles': '2 Crônicas',
    Ezra: 'Esdras',
    Nehemiah: 'Neemias',
    Esther: 'Ester',
    Job: 'Jó',
    Psalms: 'Salmos',
    Proverbs: 'Provérbios',
    Ecclesiastes: 'Eclesiastes',
    'Song of Solomon': 'Cântico dos cânticos',
    Isaiah: 'Isaías',
    Jeremiah: 'Jeremias',
    Lamentations: 'Lamentações',
    Ezekiel: 'Ezequiel',
    Daniel: 'Daniel',
    Hosea: 'Oséias',
    Joel: 'Joel',
    Amos: 'Amós',
    Obadiah: 'Abdias',
    Jonah: 'Jonas',
    Micah: 'Miquéias',
    Nahum: 'Naum',
    Habakkuk: 'Habacuc',
    Zephaniah: 'Sofonias',
    Haggai: 'Ageu',
    Zechariah: 'Zacarias',
    Malachi: 'Malaquias',
    Matthew: 'Mateus',
    Mark: 'Marcos',
    Luke: 'Lucas',
    John: 'João',
    Acts: 'Atos',
    Romans: 'Romanos',
    '1 Corinthians': '1 Coríntios',
    '2 Corinthians': '2 Coríntios',
    Galatians: 'Gálatas',
    Ephesians: 'Efésios',
    Philippians: 'Filipenses',
    Colossians: 'Colossenses',
    '1 Thessalonians': '1 Tessalonicenses',
    '2 Thessalonians': '2 Tessalonicenses',
    '1 Timothy': '1 Timóteo',
    '2 Timothy': '2 Timóteo',
    Titus: 'Tito',
    Philemon: 'Filêmom',
    Hebrews: 'Hebreus',
    James: 'Tiago',
    '1 Peter': '1 Pedro',
    '2 Peter': '2 Pedro',
    '1 John': '1 João',
    '2 John': '2 João',
    '3 John': '3 João',
    Jude: 'Judas',
    Revelation: 'Apocalipse',
    '1 Maccabees': '1 Macabeus',
    '2 Maccabees': '2 Macabeus',
    Baruch: 'Baruc',
    Judith: 'Judite',
    Sirach: 'Sirácida',
    Tobit: 'Tobias',
    Wisdom: 'Sabedoria',
    version: 'Versão',
    download: 'Baixar',
    'Available Versions': 'Versões disponíveis',
    'Believers Sword': 'Believers Sword',
    maximize: 'Maximizar',
    minimize: 'Minimizar',
    Reset: 'Reiniciar',
    General: 'Geral',
    Tabs: 'Guias',
    'Primary Colors': 'Cores primárias',

    // Colors
    default: 'padrão',
    nature: 'natureza',
    Nature: 'Natureza',
    ocean: 'oceano',
    reds: 'vermelho',
    olive: 'oliva',
    sepia: 'sépia',
    ember: 'brasa',
    lavender: 'lavanda',
    mint: 'menta',
    rose: 'rosa',
    slate: 'ardósia',
    amber: 'âmbar',
    indigo: 'índigo',
    teal: 'azul-petróleo',
    cobalt: 'cobalto',
    orchid: 'orquídea',
    forest: 'floresta',
    dawn: 'amanhecer',
    dusk: 'crepúsculo',
    nord: 'nórdico',
    midnight: 'meia-noite',
    rosepaper: 'papel-rosa',
    oceanic: 'oceânico',
    graphite: 'grafite',
    sandstone: 'arenito',
    sky: 'arco-íris',
    Sky: 'Arco-íris',
    Themes: 'Temas',
    themes: 'temas',

    Language: 'Idioma',
    No: 'Não',
    Yes: 'Sim',
    Deuterocanonical: 'Deuterocanônicos',
    "Submit Sermon": 'Enviar reflexão',
    Submit: 'Enviar',
    "Create Clip Note": 'Criar nota adesiva',
    "Update Clip Note": 'Atualizar nota adesiva',
    Save: 'Salvar',
    "Select Color": 'Selecionar cor',
    Cancel: 'Cancelar',
    "Clip Note": 'Nota adesiva',
    Confirm: 'Confirmar',
    "Are You Sure You want to remove?": 'Tem certeza que deseja remover?',
    "Add to Bookmark": 'Adicionar ao marcador',
    night: 'escuro',
    customize: 'personalizar',
    'Pick a style and color for you': 'Escolha um estilo e cor para você',
    color: 'cor do tema primário',
    theme: 'aparência',
    Donate: 'Doar',
    'Study Space': 'Estudo',
    clear: 'limpar',
    'hide-notes': 'Ocultar Notas',
    'show-notes': 'Mostrar Notas',
    "Clear Highlight": 'Limpar destaque',
    'dictionary': 'dicionário',
    'references': 'referências',
    'answered': 'respondido',
    "Believers\' Feed": "Feed dos Fiéis",
    "Daily Devotional": "Devocional Diário",
    "added": "adicionado",
    "add": "adicionar",
    "bible": "biblia",
    "Bible": "Bíblia",
    "bible version": "versão da biblia",
    "Toggle left sidebar": "Alternar barra lateral esquerda",
    "Toggle right sidebar": "Alternar barra lateral direita",
    "Toggle notes panel": "Alternar painel de notas",
    "Added": "Adicionado",
    "Download": "Baixar",
    "Import": "Importar",
    "Search versions...": "Pesquisar versões...",
    "Sign in to access your Believers Sword account. To sync your bookmarks, highlights, clip notes, and notes, download the Believers Sword mobile app and enable sync in it.": "Faça login para acessar sua conta Believers Sword. Para sincronizar seus favoritos, destaques, notas destacadas e anotações, baixe o aplicativo móvel Believers Sword e ative a sincronização nele.",
    "Sign In": "Fazer login",
    "Email Address:": "Endereço de E-mail:",
    "Email": "E-mail",
    "Password:" "Senha:",
    "Password": "Senha",
    "Create Account": "Criar conta",
    "Change Theme": "Mudar tema",
    "About": "Sobre",
    "Highlight Verse": "Destacar versículo",
    "Verse Reader": "Leitor de versículo",
    "Compare Verse": "Comparar versículo",
    "Piper TTS - Click to change": "Piper TTS - Clique para mudar",
    "Browser Text to Speech": "Navegar texto para falar",
    "Read verse number": "Ler o número do versículo",
    "Announce the verse number before reading each verse": "Anunciar o número do versículo antes de ler cada versículo",
    "No Bible versions found.": "Nenhuma versão da Bíblia foi encontrada",
    "Choose the display language for the app interface.": "Selecione o idioma para a interface do aplicativo",
    "Scale": "Escala",
    "Adjust the overall zoom of the app. Drag the slider or use the keyboard shortcuts.": "Ajuste o zoom geral do aplicativo. Arraste o controle deslizante ou use os atalhos de teclado.",
    "Reset": "Reiniciar",
    "Show or hide the deuterocanical books (e.g. Tobit, Judith, Maccabees) in the Bible list.": "Mostrar ou ocultar os livros deuterocanônicos (Ex.: Tobias, Judite, Macabeus) na lista de livros bíblicos.",
    "Updates": "Atualização",
    "Check for Updates": "Verificar por atualização",
    "You'are up to date": "Você está atualizado",
    "Choose wich tabs are visible in the navigation bar.": "Seleciona quais abas vão estar visíveis na barra de navegação.",
    "Manage and view your sermon note and outlines.": "Gerencie e visualize suas anotações e esboços de sermão.",
    "Track and organize your personal prayer requests.": "Acompanhe e organize seus pedidos de oração pessoais.",
    "Select Book":"Selecionar Livro",
    "Search Using Text": "Pesquisar usando texto",
    "Coming Soon": "Em breve",
};
export default message;
