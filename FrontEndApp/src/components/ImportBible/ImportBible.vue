...
import { NModal, NCard, NButton, NInput, NSelect, NIcon, useMessage, NSpin } from 'naive-ui';
...
// Check for duplicate whenever title changes
...
async function doImport() {
    ...
    bibleDownloadStore.isImporting = true;
    ...
    try {
        ...
        bibleDownloadStore.isImporting = false;
    }
}
...
function resetAndClose() {
    ...
    validationResult.value = null;
    duplicateError.value = null;
    bibleDownloadStore.showImportBibleModal = false;
}
</script>
<template>
    <NModal v-model:show="bibleDownloadStore.showImportBibleModal" @after-leave="resetAndClose">
        <NCard style="width: 500px" :bordered="false" size="small" role="dialog" aria-modal="true">
            <div class="mb-4">
                <h1 class="font-800 select-none m-0 capitalize">{{ $t('import') }} {{ $t('bible version') }}</h1>
            </div>
            ...
            <!-- Validation Status -->
            <div v-if="isValidating" class="flex items-center gap-2 mb-3 text-xs opacity-70">
                <NSpin size="small" />
                Validating file...
            </div>
            ...
            <div class="flex justify-end gap-2 mt-4">
                <NButton size="small" @click="resetAndClose" :disabled="isImporting">
                    Cancel
                </NButton>
